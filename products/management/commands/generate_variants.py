    ...
    def handle(self, *args, **options):
        ...
        for product in products:
            ...
            for i in range(variant_count):
                ...
                sku = f"{slugify(product.name)[:10]}-V{i+1}"

                # Adjust price slightly from base price
                price_adjustment = Decimal(str(round(random.uniform(-20, 20), 2)))
                variant_price = max(base_price + price_adjustment, Decimal("0.01"))
                ...
                variant = ProductVariant.objects.create(
                    ...
                    width=product.width + Decimal(str(round(random.uniform(-5, 5), 2))),
                ...
                total_variants += 1
                self.stdout.write(
                    f"Created variant: {variant.name} for product: {product.name}"
                )
